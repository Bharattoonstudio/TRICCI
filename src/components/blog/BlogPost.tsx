...
/**
 * Category-aware CTA that links blog readers to relevant jobs/signup pages.
 * Placed at the end of every post to create crawlable internal links.
 */
function PostJobsCTA({ category }: { category?: string }) {
  const isConsultant = category === 'For Consultants';
  const isEmployer   = category === 'Industry Insights';

  if (isConsultant) {
    return (
      <div className="mt-16 rounded-2xl border border-secondary/30 bg-secondary/5 p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        <div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 shrink-0">
          <Link
            to="/jobs"
            className="inline-flex items-center justify-center gap-2 bg-secondary text-background font-bold text-sm px-6 py-3 rounded-xl hover:opacity-90 transition-opacity whitespace-nowrap"
          >
            Browse Open Roles
          </Link>
          <Link
            to="/signup?role=consultant"
            className="inline-flex items-center justify-center gap-2 border border-secondary/40 text-foreground font-semibold text-sm px-6 py-3 rounded-xl hover:border-secondary transition-colors whitespace-nowrap"
          >
            Join as Consultant
          </Link>
        </div>
  ...
  if (isEmployer) {
    return (
      <div className="mt-16 rounded-2xl border border-primary/30 bg-primary/5 p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest text-primary mb-2">For Employers</p>
          <h3 className="text-xl font-black text-foreground mb-1" style={{ fontFamily: 'var(--font-heading)' }}>
            Hire senior talent through India's best independent consultants
          </h3>
          <p className="text-sm text-muted-foreground">
            Post a role on TRICCI and get matched with specialist consultants who know your domain.
          </p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 shrink-0">
          <Link
            to="/signup?role=employer"
            className="inline-flex items-center justify-center gap-2 bg-primary text-primary-foreground font-bold text-sm px-6 py-3 rounded-xl hover:opacity-90 transition-opacity whitespace-nowrap"
          >
            Post a Role
          </Link>
          <Link
            to="/jobs"
            className="inline-flex items-center justify-center gap-2 border border-primary/40 text-foreground font-semibold text-sm px-6 py-3 rounded-xl hover:border-primary transition-colors whitespace-nowrap"
          >
            See Live Openings
          </Link>
        </div>
  ...
  // Default — For Candidates (and any uncategorised posts)
  return (
    <div className="mt-16 rounded-2xl border border-primary/30 bg-primary/5 p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
      <div>
        <p className="text-xs font-bold uppercase tracking-widest text-primary mb-2">For Candidates</p>
        <h3 className="text-xl font-black text-foreground mb-1" style={{ fontFamily: 'var(--font-heading)' }}>
          Find your next senior role in India
        </h3>
        <p className="text-sm text-muted-foreground">
          Browse curated openings sourced by specialist consultants — no spray-and-pray, no agency markup.
        </p>
      </div>
      <div className="flex flex-col sm:flex-row gap-3 shrink-0">
        <Link
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
          ...
          <header className="mb-16 lg:mb-24">
            <div className="text-center">
              ...
              <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black text-foreground mb-10 leading-[0.95] tracking-tighter">
            ...
            <div className="blog-content prose-custom">
              ...
              <ReactMarkdown
                ...
                components={{
                  ...
                  code: ({node, ...props}) => {
                    const inline = !('data-language' in (props as Record<string, unknown>)) && !(props as Record<string, unknown>).className;
                    if (inline) {
                      return (
          ...
          <PostJobsCTA category={post.category} />
