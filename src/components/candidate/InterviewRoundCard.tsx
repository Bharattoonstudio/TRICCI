...
export default function InterviewRoundCard({
  ...
  const handleAddToCalendar = () => {
    ...
    const ics = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//TRICCI//Interview//EN',
      'BEGIN:VEVENT',
      `UID:interview-${round.id}@tricci.in`,
      `DTSTAMP:${toIcsDate(new Date())}`,
      `DTSTART:${toIcsDate(start)}`,
      `DTEND:${toIcsDate(end)}`,
      `SUMMARY:${round.roundName || 'Interview'} — ${jobTitle} at ${companyName}`,
      `DESCRIPTION:${round.roundName || 'Interview'} round for ${jobTitle} at ${companyName}.`,
      'END:VEVENT',
      'END:VCALENDAR',
    ].join('\r\n');
    ...
    a.href = url;
    a.download = `interview-${jobTitle.replace(/[^a-zA-Z0-9]/g, '-')}.ics`;
  ...
  return (
    <div className={`border rounded-xl p-4 transition-all ${
      ...
      {/* Action Button */}
      {isScheduled && (
        <button onClick={handleAddToCalendar} className="w-full mt-3 px-3 py-2 text-xs font-semibold bg-primary text-primary-foreground rounded-lg hover:bg-primary/90">
          Add to Calendar
        </button>
      )}
    </div>
