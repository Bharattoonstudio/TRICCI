/**
 * OfferModal — send an offer for a selected candidate, or record the
 * candidate's response to an already-sent offer. Works for both
 * consultant-sourced submissions and direct candidate applications —
 * pass whichever id applies.
 */
...
export default function OfferModal({ submissionId, applicationId, candidateName, onClose, onUpdated }: OfferModalProps) {
  ...
  useEffect(() => {
    const url = submissionId
      ? `/api/employer/placements/by-submission/${submissionId}`
      : `/api/employer/placements/by-application/${applicationId}`;
    fetch(url)
      .then(r => r.json())
