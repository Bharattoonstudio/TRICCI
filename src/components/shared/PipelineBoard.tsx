...
interface PipelineBoardProps {
  ...
  onManageInterview?: (card: PipelineCard) => void;
  onRequestDocuments?: (card: PipelineCard) => void;
...
function nextStageFor(card: PipelineCard): { key: string; label: string } | null {
  ...
  if (card.status === 'shortlisted') {
    return card.source === 'consultant' ? { key: 'interview', label: 'Move to Interview' } : { key: 'selected', label: 'Mark Selected' };
  }
  if (card.status === 'interview') return { key: 'selected', label: 'Mark Selected' };
  return null;
}

export default function PipelineBoard({ cards, onAdvance, onReject, onManageOffer, onManageInterview, onRequestDocuments, actioningIds }: PipelineBoardProps) {
                  ...
                  return (
                    <div key={`${card.source}-${card.id}`} className="bg-card border border-border rounded-lg p-3">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {stage.key === 'interview' && card.source === 'consultant' && onManageInterview && (
                          <button
                            onClick={() => onManageInterview(card)}
                            className="flex-1 text-[10px] font-semibold px-2 py-1 rounded border border-purple-500/30 text-purple-500 hover:bg-purple-500/10"
                          >
                            Interview Rounds
                          </button>
                        )}
                        {stage.key === 'selected' && onManageOffer && (
                          <button
                        ...
                        {(stage.key === 'shortlisted' || stage.key === 'selected') && onRequestDocuments && (
                          <button
                            onClick={() => onRequestDocuments(card)}
                            className="flex-1 text-[10px] font-semibold px-2 py-1 rounded border border-blue-500/30 text-blue-500 hover:bg-blue-500/10"
                          >
                            Documents
                          </button>
                        )}
