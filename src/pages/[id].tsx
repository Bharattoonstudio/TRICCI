...
import {
  ...
  Building2, Home, ArrowRight, Bookmark, BookOpen, ListChecks, Sparkles, Lock
} from 'lucide-react';
...
export default function JobDetailPage() {
  ...
  // AI Match Score (lightweight, auto-fetched breakdown)
  const [matchScore, setMatchScore] = useState<{
    overall: number;
    breakdown: { skills: number; experience: number; industry: number; location: number; noticePeriod: number; salary: number };
    missingSkills: string[];
    suggestion: string;
  } | null>(null);
  const [matchScoreState, setMatchScoreState] = useState<'idle' | 'loading' | 'ready' | 'unavailable'>('idle');
  ...
  // Auto-fetch a lightweight AI match score for candidates once the job loads
  useEffect(() => {
    if (!isCandidate || !job) return;
    setMatchScoreState('loading');
    fetch('/api/candidate/match-score', {
      method: 'POST',
      ...
      .then(data => {
        if (data.scored) {
          ...
          setMatchScoreState('ready');
        } else {
          setMatchScoreState('unavailable');
        }
      })
      .catch(() => setMatchScoreState('unavailable'));
  }, [isCandidate, job]);
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {job.companyHidden && (
                    <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-amber-500/15 text-amber-600 flex items-center gap-1">
                      <Lock size={10} /> Confidential
                    </span>
                  )}
                  {job.visibility === 'consultant_only' && (
                    <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-purple-500/15 text-purple-500">
                      Consultants Only
                    </span>
                  )}
                </div>
              ...
              {/* AI Match Score */}
              {isCandidate && matchScoreState === 'ready' && matchScore && (
                <div className="bg-card border border-border rounded-xl p-4 mb-8">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-xs font-bold text-muted-foreground uppercase tracking-wide flex items-center gap-1.5">
                      <Sparkles size={12} className="text-primary" /> AI Match Score
                    </p>
                    <p className={`text-xl font-black ${matchScore.overall >= 80 ? 'text-green-600' : matchScore.overall >= 60 ? 'text-amber-500' : 'text-red-500'}`}>
                      {matchScore.overall}%
                    </p>
                  </div>
                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-3">
                    {([
                      ...
                      ['Industry', matchScore.breakdown.industry],
                      ['Location', matchScore.breakdown.location],
                      ['Notice', matchScore.breakdown.noticePeriod],
                      ['Salary', matchScore.breakdown.salary],
                    ] as const).map(([label, val]) => (
                      <div key={label} className="text-center">
                        <p className="text-[9px] text-muted-foreground uppercase font-semibold mb-1">{label}</p>
                        <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${val >= 80 ? 'bg-green-600' : val >= 60 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${val}%` }} />
                        </div>
                        <p className="text-[10px] font-bold text-foreground mt-0.5">{val}%</p>
                      </div>
                    ))}
                  </div>
                  {matchScore.missingSkills.length > 0 && (
                    <p className="text-xs text-muted-foreground mb-1.5">
                      <span className="font-semibold text-foreground">Missing:</span> {matchScore.missingSkills.join(', ')}
                    </p>
                  )}
                  {matchScore.suggestion && (
                    <p className="text-xs text-muted-foreground italic">{matchScore.suggestion}</p>
                  )}
                </div>
              )}
