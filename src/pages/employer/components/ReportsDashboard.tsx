...
const ACTIVITY_TEMPLATES = [
  { id: 'weekly-hiring-summary', name: 'Weekly Hiring Summary', description: 'Applications received, shortlisted, and interviews scheduled this week', frequency: 'Weekly' },
  { id: 'monthly-placement-report', name: 'Monthly Placement Report', description: 'Positions closed, fees paid, and consultant performance', frequency: 'Monthly' },
  { id: 'pipeline-health-check', name: 'Pipeline Health Check', description: 'Stale applications, pending interviews, and offer status', frequency: 'Bi-weekly' },
  { id: 'consultant-performance', name: 'Consultant Performance', description: 'Submissions, shortlist rate, and placement rate per consultant', frequency: 'Monthly' },
];
...
export default function ReportsDashboard() {
  ...
  const [results, setResults] = useState<Record<string, { generatedAt: string; summary: Record<string, number> }>>({});
  const [lastRun, setLastRun] = useState<Record<string, string>>(() => {
    try { return JSON.parse(localStorage.getItem('tricci_report_last_run') || '{}'); } catch { return {}; }
  });
  ...
  async function runReport(id: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const updated = { ...lastRun, [id]: data.generatedAt };
        setLastRun(updated);
        localStorage.setItem('tricci_report_last_run', JSON.stringify(updated));
      }
    } finally {
      setRunningId(null);
    }
  ...
  function exportReport(id: string) {
    window.location.href = `/api/employer/reports/${id}?format=csv`;
  }
            ...
            >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <span className="text-xs text-muted-foreground">
                        Last run: {lastRun[t.id] ? new Date(lastRun[t.id]).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' }) : '—'}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="flex gap-2 shrink-0">
                  <button onClick={() => runReport(t.id)} disabled={runningId === t.id}
                    ...
                    {runningId === t.id ? <Loader2 size={12} className="animate-spin" /> : null} Run Now
                  </button>
                  <button onClick={() => exportReport(t.id)}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-muted text-muted-foreground text-xs font-semibold hover:text-foreground transition-colors">
              ...
              {results[t.id] && (
                <div className="mt-4 pt-4 border-t border-border grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {Object.entries(results[t.id].summary).map(([key, value]) => (
