...
import DocumentsModal from '@/components/employer/DocumentsModal';
...
export default function EmployerJobDetailPage() {
  ...
  const [interviewStageTarget, setInterviewStageTarget] = useState<PipelineCard | null>(null);
  const [docsTarget, setDocsTarget] = useState<PipelineCard | null>(null);
  ...
  async function actOnApplication(aid: number, status: 'shortlisted' | 'selected' | 'rejected', rejectionReason?: string) {
  ...
  return (
    <>
          ...
          {viewMode === 'pipeline' && (
            <div className="bg-card border border-border rounded-2xl p-6 mb-5">
              <PipelineBoard
                ...
                onAdvance={(card, nextStatus) => card.source === 'consultant' ? actOnSubmission(card.id, nextStatus as 'shortlisted' | 'rejected') : actOnApplication(card.id, nextStatus as 'shortlisted' | 'selected' | 'rejected')}
                ...
                onManageInterview={(card) => setInterviewStageTarget(card)}
                onRequestDocuments={(card) => setDocsTarget(card)}
              />
            </div>
          )}
      ...
      {offerTarget && (
        <OfferModal
          submissionId={offerTarget.source === 'consultant' ? offerTarget.id : undefined}
          applicationId={offerTarget.source === 'direct' ? offerTarget.id : undefined}
          candidateName={offerTarget.candidateName}
          onClose={() => setOfferTarget(null)}
          onUpdated={load}
        />
      )}
      {interviewStageTarget && (
        <InterviewModal
          submissionId={interviewStageTarget.id}
          candidateName={interviewStageTarget.candidateName}
          onClose={() => setInterviewStageTarget(null)}
        />
      )}
      {docsTarget && (
        <DocumentsModal
          entityType={docsTarget.source === 'consultant' ? 'submission' : 'application'}
