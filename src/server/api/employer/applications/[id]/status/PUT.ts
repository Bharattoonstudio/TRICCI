...
import { candidateApplication, job, user, candidateProfile, placement } from '@/server/db/schema.js';
...
const VALID_STATUSES = ['shortlisted', 'selected', 'rejected', 'placed'] as const;
  ...
  try {
    ...
    const [row] = await db
      .select({
        ...
        currentCTC: candidateProfile.currentCTC,
        feePercent: job.feePercent,
        paymentTermDays: job.paymentTermDays,
      })
    ...
    // Record a placement when a direct application is marked 'selected' —
    // same timing as the consultant flow, so an offer can be sent before
    // the hire is actually confirmed. Same fee as a consultant placement,
    // but 100% stays with the platform — there's no consultant to split it with.
    if (status === 'selected') {
      try {
        const [existingPlacement] = await db.select({ id: placement.id, paymentStatus: placement.paymentStatus })
          .from(placement).where(eq(placement.applicationId, id)).limit(1);

        if (!existingPlacement) {
          const ctcLpa = row.currentCTC ? row.currentCTC / 100000 : null; // stored as paise-equivalent
          const feePercent = row.feePercent ?? null;
          ...
          await db.insert(placement).values({
            ...
            feePercent,
            feeAmountLpa,
            platformFeePercent: feePercent, // full fee stays with platform — no consultant
            consultantFeePercent: 0,
            consultantFeeAmountLpa: 0,
            paymentTermDays: row.paymentTermDays ?? 45,
            paymentStatus: 'pending',
            placedAt: new Date(),
          });
          console.log(`[placement] recorded for direct application ${id} — ${row.candidateName} at ${row.company}`);
        }
      } catch (placementErr) {
        // Non-fatal — don't fail the status update if placement recording fails
        console.error('[placement] failed to record direct-application placement:', placementErr);
      }
    }
    ...
    if (row.candidateEmail) {
      ...
      const subject = isShortlisted
        ...
        : isSelected
        ? `🎉 You've been selected for ${row.jobTitle} at ${row.company}`
        : isRejected
      ...
      const body = isShortlisted
        ...
        : isSelected
        ? `Great news! You've been selected for <strong style="color:${TEXT};">${row.jobTitle}</strong> at <strong style="color:${TEXT};">${row.company}</strong>. The employer will be reaching out with an offer shortly.`
        : isRejected
        ...
        : `Your application for <strong style="color:${TEXT};">${row.jobTitle}</strong> at <strong style="color:${TEXT};">${row.company}</strong> has been updated.`;

      const statusColor = isRejected ? '#ef4444' : ORANGE;
      const statusLabel = isShortlisted ? '⭐ Shortlisted' : isSelected ? '🎉 Selected' : isRejected ? '❌ Not Selected' : 'Update';
