/**
 * GET /api/employer/submissions/:id/documents
 * Employer sees every document request and every uploaded document for
 * this submission, regardless of who uploaded it.
 */
...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    const requests = await db.select().from(documentRequest)
      .where(and(eq(documentRequest.entityType, 'submission'), eq(documentRequest.entityId, String(entity.id))))
      .orderBy(desc(documentRequest.createdAt));

    const documents = await db.select().from(documentSubmission)
      .where(and(eq(documentSubmission.entityType, 'submission'), eq(documentSubmission.entityId, String(entity.id))))
      .orderBy(desc(documentSubmission.uploadedAt));

    res.json({ requests, documents, candidateName: entity.candidateName });
  } catch (err) {
    console.error('[employer.submissions.documents.get] ERROR:', err);
