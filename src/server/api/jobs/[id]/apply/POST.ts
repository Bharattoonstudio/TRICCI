...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    // P0 #2: Verify candidate has granted consent before applying
    ...
    if (candUser?.consentStatus !== 'granted') {
      return res.status(403).json({
        error: 'consent_required',
        message: 'You must grant consent before applying to jobs. Please visit your profile settings.',
