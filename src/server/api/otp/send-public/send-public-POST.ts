...
if (hasSmsKey) {
  try {
    ...
    if (!response.ok) {
      console.error('[otp.send-public] SMS delivery failed with status:', response.status);
    }
  } catch (smsErr) {
...
async function handler(req: Request, res: Response) {
  const { phone, email, name, role, purpose, type } = req.body;
  ...
  const recentOtps = await db
    ...
    .where(
      and(
        eq(otpStore.phone, sanitizedPhone),
  ...
  try {
    ...
    await db.insert(otpStore).values({
      ...
      email,
      otp,
      purpose: type === 'signup' ? 'signup' : 'login',
      expiresAt,
    });
    ...
    if (hasSmsKey) {
      try {
        await fetch('https://www.fast2sms.com/dev/bulkV2', {
          ...
          console.error('[otp.send-public] SMS delivery failed:', err instanceof Error ? err.message : '');
          // Continue anyway - Email sent successfully
        });
      } catch (smsErr) {
...
// Export with rate limiting middleware applied for authenticated endpoints ONLY
// For public signup, we bypass authentication check
export default (req: Request, res: Response) => {
  // Public endpoint - no authentication required
  // Only IP-based rate limiting for abuse prevention
