...
import { submission, job, user, placement, candidateProfile, auditLog, commissionConfig } from '@/server/db/schema.js';
...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    // ── P0 #3: ATOMIC placement creation on selection ──────────────────────────
    // All operations wrapped in transaction to ensure atomicity
    // If ANY step fails, entire placement creation is rolled back
      ...
      try {
        await db.transaction(async (trx) => {
          ...
          if (existing.length === 0) {
            // P0 #4: Fetch platform fee config instead of hardcoded value
            const [feeConfig] = await trx
              .select({ platformFeePct: commissionConfig.platformFeePct })
              .from(commissionConfig)
              .limit(1);

            const PLATFORM_CUT = feeConfig?.platformFeePct ?? 2; // Fallback to 2 if not configured
            ...
            // P0 #4: Platform cut now comes from config, consultant gets the rest
            ...
            // P0 #3: All operations in same transaction
            // Step 1: Create placement record
            const [placementRecord] = await trx
              .insert(placement)
              .values({
                submissionId: id,
                jobId: row.jobId,
            ...
            console.log(`[P0#3] placement created (atomic) for submission ${id} — ${row.candidateName} at ${row.company}`);

            // Step 2: Log placement creation to audit
            await trx
              .insert(auditLog)
              .values({
                ...
                entityId: String(placementRecord.id),
            ...
            // Mark existing placement as paid
            await trx
              .update(placement)
              .set({ paymentStatus: 'paid' })
              .where(eq(placement.submissionId, id));

            console.log(`[P0#3] placement marked paid (atomic) for submission ${id}`);
          }
        });
      } catch (placementErr) {
        // P0 #3: No longer non-fatal — placement creation is critical
        console.error('[P0#3] CRITICAL: placement creation failed:', placementErr);
        // Log to audit for investigation
        await db
          .insert(auditLog)
          .values({
            userId: session.user.id,
            action: 'placement_creation_failed',
            entityType: 'submission',
            entityId: String(id),
            oldValue: JSON.stringify({ status: 'pending', submissionId: id }),
            newValue: JSON.stringify({ error: String(placementErr), timestamp: new Date() }),
            timestamp: new Date(),
          })
          .catch(() => {});

        // Return error to caller — placement is critical and must not fail silently
        return res.status(500).json({
          error: 'placement_creation_failed',
          message: 'Failed to create placement record. This is a critical operation and must succeed. Please retry or contact support.',
          details: process.env.NODE_ENV === 'development' ? String(placementErr) : undefined,
        });
      }
    }
